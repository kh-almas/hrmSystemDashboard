...
const CompanyHoliday = () => {
    ...
    const deleteShift = id => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(`/hrm-system/holiday/${id}`)
                    .then(info => {
                        if(info?.status == 200)
                        {
                            Swal.fire(
    ...
    return (
        <>
            ...
            {
                oldData ?
                    <HolidayUpdateModal allHolidayReFetch={allHolidayReFetch} oldData={oldData} dataUpdateModal={dataUpdateModal} dataUpdateToggle={dataUpdateToggle}></HolidayUpdateModal>
                    : ''
            }
        </>
    );
