...
const EditProduct = () => {
    ...
    const [isRowMaterialValue, setIsRowMaterialValue] = useState();
    ...
    useEffect(() => {
        ...
        if(singleProductData?.measurementUnit) {
            setMeasurementUnit({value: singleProductData?.measurementUnit, label: singleProductData?.measurementUnit})
        }

        if(singleProductData?.weightUnit) {
            setWeightUnit({value: singleProductData?.weightUnit, label: singleProductData?.weightUnit})
        }

        if(singleProductData?.unitId) {
            const filteredUnit = allUnitType?.find(singleItem => singleItem?.value == singleProductData?.unitId)
            setUnitType(filteredUnit);
        }

        if(singleProductData?.brandId) {
            const filteredBrand= allBrand?.find(singleItem => singleItem?.value == singleProductData?.brandId)
            setBrandValue(filteredBrand);
        }

        if(singleProductData?.modelId) {
            const filteredModel= allModel?.find(singleItem => singleItem?.value == singleProductData?.modelId)
            setSingleModel(filteredModel);
        }

        if(singleProductData?.modelId) {
            const filteredBarcode= barcodeTypeData?.find(singleItem => singleItem?.value == singleProductData?.barcodeType)
            setBarcodeType(filteredBarcode);
        }

        if(singleProductData?.taxType) {
            const filteredTaxType= taxTypeData?.find(singleItem => singleItem?.value == singleProductData?.taxType)
        ...
        setValue('p_weight', singleProductData?.productWeight);
        setValue('package_height', singleProductData?.packageHeight);
    ...
    const submitAddProductForm = (data) => {
        ...
        data.alert_quantity = allStoredValue.alert_quantity ? allStoredValue.alert_quantity :  data.alert_quantity;
        data.opening_stock_quantity = allStoredValue.opening_stock_quantity ? allStoredValue.opening_stock_quantity : data.opening_stock_quantity;
    ...
    return (
        <div>
            <Breadcrumb parent="Inventory management" title="Edit Product"/>
                                                        ...
                                                        {type !== "Variant" ? (
                                                            <div className={"mt-3"}>
                                                                <TextField
                                                                    ...
                                                                    size='small'
                                                                    type={'text'}
                                                                    value={`${singleProductData?.productSku}`}
                                                                    label={'SKU'}
                            ...
                            <Container fluid={true}>
                                ...
                                <Accordion defaultActiveKey="0">
                                    <div className="default-according" id="accordion">
                                        {
                                            makeProductOptions?.map((singleOptions, index) =>
                                                <Card key={index}>
                                                    ...
                                                    <Collapse
                                                        ...
                                                        <CardBody className="p-3">
                                                            ...
                                                            <div className="d-flex justify-content-end">
                                                                <button
                                                                    onClick={() => addNewRowForOptionValues(singleOptions?.value)}
                                                                    className="btn btn-outline-primary btn-xs mx-3 mt-1"
